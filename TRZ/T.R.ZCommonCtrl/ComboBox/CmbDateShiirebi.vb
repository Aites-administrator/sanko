...
#Region "コンストラクタ"
  ...
  Public Sub New()
    ...
    InitCmb()
    ' フォーカス時、表示メッセージ設定
    MyBase.SetMsgLabelText("仕入日を選択してください。")
...
#Region "パブリック"
  ...
  Public Function SqlSelListSrc(prmCode As String) As String
    ...
    sql &= " SELECT Format(KEIRYOBI,'" & CODE_FORMAT & "') AS ItemCode  "
    sql &= " FROM CUTJ "
    sql &= " WHERE NSZFLG <= 6 "
    sql &= "   AND KEIRYOBI > '" & DateAdd(DateInterval.Month, -1, dt).ToString("yyyy/MM/dd") & "'"
    sql &= " GROUP BY KEIRYOBI "
    sql &= " ORDER BY KEIRYOBI DESC; "
